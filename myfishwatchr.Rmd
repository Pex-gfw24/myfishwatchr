...
I had to update my `R.version today (24/10/24)` over suspicious that this may be the issue causing errors in line 660 inwards on plotting vessel tracks. 
# Global Options
...
```{r}
#git_user <- system("git config --global user.name", intern = TRUE)  #..Verifying the github account...#
#git_email <- system("git config --global user.email", intern = TRUE)  #..Verifying the github account...#
```
...
# Step 3 
I ran into some issues with BigQuery authentication; this was probably becasue of the change from my old personal github account to my new gfw github account. So I used the piece of code below to re-authentication with bigquery and was able to run the code in lines `197 - 212` 
`library(bigrquery)`
...
Tried this code, `remotes::install_github("GlobalFishingWatch/fishwatchr")` but somehow the package `fishwatchr` was not being loaded in so i removed the `remotes::` and just ran `install_github("GlobalFishingWatch/fishwatchr")` which was then successful in loading the package in.

```{r eval = FALSE}

# Install fishwatchr
install_github("GlobalFishingWatch/fishwatchr")
```
...
I did not run this piece of code becase I already have the font installed in my Macbook machnine.
```{r eval = FALSE}
...
I also had to install `tidyverse` and `glue` packages  
...
Ran into some issues with my bigquesry `auth` so i had to run this piece of code: `library(bigrquery) bq_auth()`. That send me back ot the browser and I aunthenticated using my gfw google account and masking sure that the `select all` box is checked before continuing.  
...
Note: After successfully updating my R.version to 4.4.1 today (24/10/24), I was able to run the chunk of code below successfully. 
